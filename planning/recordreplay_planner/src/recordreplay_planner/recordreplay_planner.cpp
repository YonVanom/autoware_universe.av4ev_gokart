...
bool RecordReplayPlanner::record_state(const State & state_to_record)
{
  ...
  auto distance_sq =
    (state_to_record.pose.position.x - previous_state.pose.position.x) *
      (state_to_record.pose.position.x - previous_state.pose.position.x) +
    (state_to_record.pose.position.y - previous_state.pose.position.y) *
      (state_to_record.pose.position.y - previous_state.pose.position.y);
...
std::size_t RecordReplayPlanner::get_closest_state(const State & current_state)
{
  ...
  const auto distance_from_current_state = [this, &current_state](State & other_state) {
    const auto s1 = current_state, s2 = other_state;
    return (s1.pose.position.x - s2.pose.position.x) * (s1.pose.position.x - s2.pose.position.x) +
           ...
           m_heading_weight * std::abs(tf2::getYaw(s1.pose.orientation - s2.pose.orientation));
  };
...
const Trajectory & RecordReplayPlanner::from_record(const State & current_state)
{
  ...
  if (!m_enable_loop) {
    ...
    publication_len = std::min(record_length - m_traj_start_idx, trajectory.points.max_size());
  } else {
    publication_len = std::min(record_length, trajectory.points.max_size());
  }
  ...
  if (!m_enable_loop || m_traj_start_idx < m_traj_end_idx) {
    ...
    for (std::size_t i = 0; i < publication_len; ++i) {
      ...
      trajectory.points[i] = m_record_buffer[m_traj_start_idx + i];
    ...
    for (std::size_t i = m_traj_start_idx; i < record_length; ++i) {
      ...
      trajectory.points[idx] = m_record_buffer[i];
...
void RecordReplayPlanner::readTrajectoryBufferFromFile(const std::string & replay_path)
{
  ...
  for (size_t i = 0; i < file_data.size(); ++i) {
    ...
    for (size_t j = 0; j < file_data.at(i).size(); ++j) {
      ...
      if (map.at("t_sec") == _j) {
        s.time_from_start.sec = std::stoi(file_data.at(i).at(j));
      } else if (map.at("t_nanosec") == _j) {
        s.time_from_start.nanosec = static_cast<uint32_t>(std::stoi(file_data.at(i).at(j)));
      } else if (map.at("x") == _j) {
        s.pose.position.x = std::stof(file_data.at(i).at(j));
      } else if (map.at("y") == _j) {
        s.pose.position.y = std::stof(file_data.at(i).at(j));
      } else if (map.at("orientation_x") == _j) {
        s.pose.orientation.x = std::stof(file_data.at(i).at(j));
      } else if (map.at("orientation_y") == _j) {
        s.pose.orientation.y = std::stof(file_data.at(i).at(j));
      } else if (map.at("orientation_z") == _j) {
        s.pose.orientation.z = std::stof(file_data.at(i).at(j));
      } else if (map.at("orientation_w") == _j) {
        s.pose.orientation.w = std::stof(file_data.at(i).at(j));
      } else if (map.at("longitudinal_velocity_mps") == _j) {
        s.longitudinal_velocity_mps = std::stof(file_data.at(i).at(j));
      } else if (map.at("lateral_velocity_mps") == _j) {
        s.lateral_velocity_mps = std::stof(file_data.at(i).at(j));
      } else if (map.at("acceleration_mps2") == _j) {
        s.acceleration_mps2 = std::stof(file_data.at(i).at(j));
      } else if (map.at("heading_rate_rps") == _j) {
        s.heading_rate_rps = std::stof(file_data.at(i).at(j));
      } else if (map.at("front_wheel_angle_rad") == _j) {
        s.front_wheel_angle_rad = std::stof(file_data.at(i).at(j));
      } else if (map.at("rear_wheel_angle_rad") == _j) {
        s.rear_wheel_angle_rad = std::stof(file_data.at(i).at(j));
      }
    }
    record_state(s);
  }
  ...
  const float64_t & angle_thresh) const
{
  ...
  const auto & ego_state = current_state;
  ...
  const auto angle_diff_rad =
    std::abs(tf2::getYaw(ego_state.pose.orientation - goal_state.pose.orientation));
...
bool8_t RecordReplayPlanner::is_loop(const float64_t & distance_thresh) const
{
  ...
  if (get_record_length() > 1) {
    const auto & goal_state = m_record_buffer.front();
    const auto & start_state = m_record_buffer.back();
    const auto distance2d = std::hypot(
